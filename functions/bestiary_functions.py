...
class Monster():
    ...
    def save_to_file(self):
        ...
        elif bestiary_flag == "random":
            file_path = RANDOM_FILE_PATH
        elif bestiary_flag == "required":
...
def random_encounters_page(root, left_frame, right_frame):
    clear_widgets(left_frame)
    clear_widgets(right_frame)
    global bestiary_flag
    ...
    button_frame = tk.Frame(right_frame)
    button_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
    for label in RANDOM_ENCOUNTERS_BUTTON_LABELS:
        btn = tk.Button(button_frame, text=label, command=lambda l=label: on_button_click(l, root, left_frame, right_frame))
        btn.pack(**BUTTON_PACK_OPTIONS)
...
def required_encounters_page(root, left_frame, right_frame):
    ...
    button_frame = tk.Frame(right_frame)
    button_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
    for label in REQUIRED_ENCOUNTERS_BUTTON_LABELS:
        btn = tk.Button(button_frame, text=label, command=lambda l=label: on_button_click(l, root, left_frame, right_frame))
        btn.pack(**BUTTON_PACK_OPTIONS)
...
def add_monster(root, left_frame=None, right_frame=None):
    ...
    if bestiary_flag == "archive":
        ...
        add_monster_flag = RANDOM_FILE_PATH
    elif bestiary_flag == "required":
        add_monster_flag = REQUIRED_FILE_PATH
    ...
    def on_submit():
        ...
        if left_frame and right_frame:
            if bestiary_flag == "archive":
                ...
                random_encounters_page(root, left_frame, right_frame)
            elif bestiary_flag == "required":
                required_encounters_page(root, left_frame, right_frame)
...
def move_monster_to_random(root, left_frame=None, right_frame=None):
    global bestiary_flag
    if bestiary_flag == "archive":
        file_path_from = ARCHIVE_FILE_PATH
    elif bestiary_flag == "required":
        file_path_from = REQUIRED_FILE_PATH

    popup = tk.Toplevel(root)
    popup.title(f"Move monster from {bestiary_flag}")

    instr_label = tk.Label(popup, text="Please type the name of the monster you wish to move to Random Encounters.")
    instr_label.pack(pady=10)
    ...
    def on_submit():
        ...
        if os.path.exists(file_path_from):
            with open(file_path_from, "r") as f:
                content = f.read().strip()
                if content:
                    monster_list_from = json.loads(content)

        found = False
        for mon in monster_list_from:
            if mon["name"].lower() == name_val.lower():
                transferred_monster = mon
                monster_list_from.remove(mon)
                with open(file_path_from, "w") as f:
                    json.dump(monster_list_from, f, indent=4)
                monster_list_to = []
                if os.path.exists(RANDOM_FILE_PATH):
                    with open(RANDOM_FILE_PATH, "r") as f:
                        content = f.read().strip()
                        if content:
                            monster_list_to = json.loads(content)

                for mon in monster_list_to:
                    if mon["name"] == name_val:
                ...
                bestiary_flag = "random"
                new_monster.save_to_file()
                bestiary_flag = temp_flag
                popup.destroy()
                show_error(f"{name_val} has been moved to random.", root)
...
def move_monster_to_archive(root, left_frame=None, right_frame=None):
    global bestiary_flag
    if bestiary_flag == "random":
        file_path_from = RANDOM_FILE_PATH
    elif bestiary_flag == "required":
        file_path_from = REQUIRED_FILE_PATH
    ...
    instr_label = tk.Label(popup, text="Please type the name of the monster you wish to move to Archive.")
    instr_label.pack(pady=10)
    ...
    def on_submit():
        ...
        for mon in monster_list_from:
            if mon["name"].lower() == name_val.lower():
                ...
                bestiary_flag = temp_flag
                popup.destroy()
                show_error(f"{name_val} has been moved to archive.", root)
...
def move_monster_to_required(root, left_frame=None, right_frame=None):
    global bestiary_flag
    if bestiary_flag == "archive":
        file_path_from = ARCHIVE_FILE_PATH
    elif bestiary_flag == "random":
    ...
    instr_label = tk.Label(popup, text="Please type the name of the monster you wish to move to Required Encounters.")
    instr_label.pack(pady=10)
    ...
    def on_submit():
        ...
        for mon in monster_list_from:
            if mon["name"].lower() == name_val.lower():
                ...
                bestiary_flag = temp_flag
                popup.destroy()
                show_error(f"{name_val} has been moved to required.", root)
